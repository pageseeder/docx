...
<!--
 Match w:p inside of a list; Creates a item for the current paragraph and checks if there are forther paragraphs to create list items or next level of lists
 create list item 
 -->
   ...
   <xsl:template match="w:p[matches(w:pPr/w:pStyle/@w:val,$numbering-paragraphs-list-string) and not(w:pPr/w:numPr/w:numId/@w:val = '0') and fn:get-psml-element(w:pPr/w:pStyle/@w:val) = '']" mode="content">
    ...
    <xsl:variable name="previous-heading-levels">
      <xsl:value-of select="fn:get-preceding-heading-level-from-element(.)" />
    </xsl:variable>
...
<!--                 <xsl:message>3</xsl:message> -->
      ...
      <xsl:otherwise>
        <para>
<!--              <xsl:message>1::<xsl:value-of  select="number($current-level)"/>-<xsl:value-of  select="number($current-num-id)"/></xsl:message> -->
...
<!--                   <xsl:message>3::<xsl:value-of  select="$has-numbering-format"/></xsl:message> -->
  ...
  <xsl:template match="w:p[w:pPr/w:numPr[w:ilvl][w:numId] and not(matches(w:pPr/w:pStyle/@w:val,$numbering-paragraphs-list-string)) and not(w:pPr/w:numPr/w:numId/@w:val = '0') and fn:get-psml-element(w:pPr/w:pStyle/@w:val) = '']" mode="content">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <para>
<!--              <xsl:message>2::<xsl:value-of  select="number($current-level)"/>-<xsl:value-of  select="$abstractNumId"/></xsl:message> -->
          <xsl:attribute name="indent" select="number($current-level) - number(fn:get-preceding-heading-level-from-element(.)) + 1" />
          <xsl:choose>
            <xsl:when test="not($is-bullet) and fn:get-numbered-paragraph-value($current-level + 1)='numbering'">
              <xsl:attribute name="numbered" select="'true'" />
            ...
            <xsl:when test="not($is-bullet) and fn:get-numbered-paragraph-value($current-level + 1)='prefix' and $list-paragraphs/w:p[@id = current()/@id]">
<!--                  <xsl:message>4::<xsl:value-of  select="$has-numbering-format"/></xsl:message> -->
              <xsl:if test="$has-numbering-format and fn:get-numbering-value-from-paragraph-style(.,$style-name) != ''">
                <xsl:attribute name="prefix" select="fn:get-numbering-value-from-paragraph-style(.,$style-name)" />
              </xsl:if>
            </xsl:when>
  ...
  <xsl:template match="w:p" mode="insidelist">
    ...
    <item>
      ...
      <xsl:for-each select="$nested-list">
        <xsl:variable name="nlist-id" select="fn:get-abstract-num-id-from-element(.)" />
        <xsl:call-template name="list">
          <xsl:with-param name="abstract-id"
            select="$abstract-num-id" />
...
-->
  ...
  <xsl:template name="list">
    ...
    <xsl:param name="id"/>
<!--     <xsl:message select="$id"></xsl:message> -->
    <xsl:variable name="style-name" select="w:pPr/w:pStyle/@w:val"/>
<!--     <xsl:variable name="id" select="@id"/> -->
    <xsl:choose>
      <xsl:when
        test="$numbering-document/w:numbering/w:abstractNum[@w:abstractNumId=$abstract-id]/w:lvl[@w:ilvl=$level]/w:numFmt/@w:val='bullet'">
        <list>
...
<!--           <xsl:message><xsl:value-of select="fn:return-pageseeder-numbering-style($numbering-document/w:numbering/w:abstractNum/w:lvl[w:pStyle[@w:val=$style-name]]/w:numFmt/@w:val)"/>::::<xsl:value-of select="$pageseeder-list-types"/></xsl:message> -->
<!--           <xsl:if test="$convert-to-list-roles"> -->
            <xsl:choose>
              <xsl:when test="fn:return-pageseeder-numbering-style($abstract-id,$level,$style-name)">
                <xsl:sequence select="fn:return-pageseeder-numbering-style($abstract-id,$level,$style-name)"/>
<!--                 <xsl:attribute name="type" select="fn:return-pageseeder-numbering-style($numbering-document/w:numbering/w:abstractNum/w:lvl[w:pStyle[@w:val=$style-name]]/w:numFmt/@w:val)"/> -->
              </xsl:when>
